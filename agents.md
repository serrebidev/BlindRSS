...
- Mandatory Windows release-build protocol: use `.\build.bat release` when producing the Windows release package locally.
...
- `./build.sh build` is for local macOS packaging only.
...
8. Releases: use `.\build.bat release` for Windows release builds; GitHub publication itself may be performed from any OS after approved artifacts exist (details in `build.md`).
