...
9. Release publication: every release MUST be published (not draft) and marked as "Latest" before the build flow exits. The Windows updater queries GitHub's `/releases/latest`, which silently skips drafts, so a draft release means no users get the update. `build.bat release` enforces this with an explicit `gh release edit --draft=false --latest` step; do not remove it, and apply the same guard to any new release path.
